...
def loadImagesBoundaries(TRAIN_PATH, IMG_HEIGHT, IMG_WIDTH, IMG_CHANNELS):
	...
	for n,id_ in tqdm(enumerate(train_ids),total=len(train_ids)):
		...
		img = resize(img, (IMG_HEIGHT, IMG_WIDTH), mode = 'constant', preserve_range=True)
		X_train[n] = img
		mask_allBounds = np.zeros((IMG_HEIGHT, IMG_WIDTH),dtype=np.float32)
		mask_allInner = np.zeros((IMG_HEIGHT, IMG_WIDTH),dtype=np.float32)
		for mask_file in next(os.walk(path + '/masks/'))[2]:
			mask_ = imread(path+'/masks/'+mask_file) #read each mask for the image
			mask_ = resize(mask_,(IMG_HEIGHT,IMG_WIDTH),mode='constant',preserve_range=True)
			density = np.zeros((IMG_HEIGHT, IMG_WIDTH),dtype=np.float32)
			
			if(np.amax(mask_)>0):
				mask_boundary = find_boundaries(mask_,mode='inner')
				mask_allBounds = np.maximum(mask_allBounds,mask_boundary)
				...
				for i in range(minX,maxX+1,1):
					for j in range(minY,maxY+1,1):
						density[i,j] = 100*gaussValues[i-int(center[0])+maxD]*gaussValues[j-int(center[1])+maxD]
				mask_allInner= mask_allInner + density #combine all mask files into one
				
		Y_train[n,:,:,1] = mask_allBounds
		Y_train[n,:,:,0] = mask_allInner
